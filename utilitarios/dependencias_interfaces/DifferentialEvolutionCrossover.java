...
import dependencias_interfaces.BoundedRandomGenerator;
...
public class DifferentialEvolutionCrossover implements CrossoverOperator<IntegerSolution> {
  ...
  /* Setters */
  public void setCurrentSolution(IntegerSolution integerSolution) {
    this.currentSolution = integerSolution;
  }
    ...
    if (crossoverType.equals(DE_CROSSOVER_TYPE.BIN)) {
      for (int j = 0; j < numberOfVariables; j++) {
        if (crRandomGenerator.getRandomValue(0.0, 1.0) < cr || j == jrand) {
          ...
          child.setVariableValue(j, (int)value);
        }
      }
        ...
        .forEach(
            i ->
                solution.setVariableValue(
                    i,
                    (int) solutionRepair.repairSolutionVariableValue(
  ...
  private double bestMutation(Double[][] parent, int index, int numberOfDifferenceVectors) {
    ...
    if (numberOfDifferenceVectors == 1) {
      return bestSolution.getVariableValue(index) + f * (parent[0][index] - parent[1][index]);
    } else if (numberOfDifferenceVectors == 2) {
  ...
  private double bestRandToBestMutation(Double[][] parent, int index) {
    ...
    return currentSolution.getVariableValue(index)
        + f * (bestSolution.getVariableValue(index) - currentSolution.getVariableValue(index))
        + f * (parent[0][index] - parent[1][index]);
  }
}
