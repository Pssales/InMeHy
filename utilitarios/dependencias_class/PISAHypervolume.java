...
import dependencias_abstract.Hypervolume;
...
public class PISAHypervolume<S extends Solution<?>> extends Hypervolume<S> {
  ...
  public BigDecimal evaluateModificado(List<S> paretoFrontApproximation) {
		 
	    if (paretoFrontApproximation == null) {
	      throw new JMetalException("The pareto front approximation is null") ;
	    }

	    return hypervolumeModificado(new ArrayFront(paretoFrontApproximation), referenceParetoFront);  }
...
private BigDecimal surfaceUnchangedToModificado(double[][] front, int noPoints, int objective) {
	    
	  	int i;
	  	
	  	BigDecimal minValue = new BigDecimal(0.0, MathContext.UNLIMITED);
	    BigDecimal value = new BigDecimal(0.0, MathContext.UNLIMITED);

	    if (noPoints < 1) {
	      new JMetalException("run-time error");
	    }
	    
	    BigDecimal aux1 = new BigDecimal(front[0][objective], MathContext.UNLIMITED);
	    minValue = aux1;
	    
	    for (i = 1; i < noPoints; i++) {
	    	
	      BigDecimal aux2 = new BigDecimal(front[i][objective], MathContext.UNLIMITED);
		  value = aux2;
...
public BigDecimal calculateHypervolumeModificado(double[][] front, int noPoints, int noObjectives) {
		
	  
	    int n;
	    
	    BigDecimal volume = new BigDecimal(0.0, MathContext.UNLIMITED);
	    BigDecimal distance = new BigDecimal(0.0, MathContext.UNLIMITED);
	    
	    n = noPoints;
	    while (n > 0) {
	      int nonDominatedPoints;
	      
	      BigDecimal tempVolume = new BigDecimal(0.0, MathContext.UNLIMITED);
		  BigDecimal tempDistance = new BigDecimal(0.0, MathContext.UNLIMITED);
	      ...
	      if (noObjectives < 3) {
	        ...
	        BigDecimal aux = new BigDecimal(front[0][0], MathContext.UNLIMITED);
	        tempVolume = aux;
	        
	      } else {
	        tempVolume = calculateHypervolumeModificado(front, nonDominatedPoints, noObjectives - 1);
	      }

	      tempDistance = surfaceUnchangedToModificado(front, n, noObjectives - 1);
	      
	      BigDecimal eq1 = new BigDecimal(0.0, MathContext.UNLIMITED);
	      eq1 = eq1.add(tempDistance);
	      eq1 = eq1.subtract(distance);
	      
	      BigDecimal eq2 = tempVolume.multiply(eq1);
	      
	      volume = volume.add(eq2);
	      ...
	      n = reduceNondominatedSet(front, n, noObjectives - 1, Double.parseDouble(d));
	...
	private BigDecimal hypervolumeModificado(Front front, Front referenceFront) {
	    ...
	    return this.calculateHypervolumeModificado(FrontUtils.convertFrontToArray(invertedFront),
