...
public abstract class AbstractGeneticAlgorithm<S, Result> extends AbstractEvolutionaryAlgorithm<S, Result> {
  ...
  protected List<S> createInitialPopulation() {
	...
	//S newIndividualcopy = null;
	//GerenciadorPopulacao gp = GerenciadorPopulacao.getInstance();
    List<S> population = new ArrayList<>(getMaxPopulationSize());
    for (int i = 0; i < getMaxPopulationSize(); i++) {
      S newIndividual = getProblem().createSolution();
      //newIndividualcopy = newIndividual;
      population.add(newIndividual);
    }
    
    S ultimo = getProblem().createSolution();
    population.add(ultimo);
    //population.add(newIndividualcopy);
    return population;
  }
