...
/*
 * Setup circular double-linked list in each dimension.
 * It considers only enable solutions.
 */

static dlnode_t *
setup_cdllist2(double *data, int *enable, int d, int *n)
{
    dlnode_t *head;
    dlnode_t **scratch;
    int i, j, cont = 1;
    ...
    for (i = 1; i <= *n; i++) { // data[i*d*]
        if(enable[i-1]) {
          head[cont].x = data + i*d; //head[i-1].x + d;/* this will be fixed a few lines below... */
          head[cont].ignore = 0;
          head[cont].next = head[cont-1].next + d;    //head[i-1].next + d;
          head[cont].prev = head[cont-1].prev + d;    //head[i-1].prev + d;
          head[cont].tnode = head[cont-1].tnode + 1;  //head[i-1].tnode + 1;
#if VARIANT >= 2
          head[cont].area = head[cont-1].area + d;    //head[i-1].area + d;
#endif
#if VARIANT >= 3
          head[cont].vol = head[cont-1].vol + d;    //head[i-1].vol + d;
          cont++;
        }
#endif
    }
...
/*
  ...
  that are needed to calculate the hypervolume. It considers only
  enable solutions.
*/
static int
...
double hv_filter(double *data, int *enable, int n, int d, const double *ref)
{
    dlnode_t *list;
    double hyperv;
    double * bound = NULL;
    ...
    list = setup_cdllist2(data, enable, d, &n);
    n = filter(list, d, n, ref);
...
double EMO_hv2_contribution(double *deltahv, double *data, int *enable, int n, const double *ref, int d) {
  ...
  totalhv = hv_filter(data, enable, n, d, ref);

  for(i = 0; i < n; i++) { 
    if(enable[i]) {
      enable[i] = 0;
      deltahv[i] = totalhv - hv_filter(data, enable, n, d, ref);
      enable[i] = 1;
    }
    else {
      deltahv[i] = DBL_MAX;
